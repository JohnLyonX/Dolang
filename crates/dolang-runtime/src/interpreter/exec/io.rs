...
pub(super) fn handle_file_write_stmt(
    ...
    match mode_str.as_deref() {
        Some("DEL") => {
            match context.call_intrinsic(ids::FS_DELETE, &[DolangValue::Str(path_str)]) {
                Ok(_) => Flow::Normal,
                Err(err) => Flow::Err(err),
            }
        }
...
pub(super) fn handle_file_read_stmt(
    ...
    let is_dir = match context.call_intrinsic(ids::FS_IS_DIR, &[DolangValue::Str(path_str.clone())])
    {
    ...
    let exists = match context.call_intrinsic(ids::FS_EXISTS, &[DolangValue::Str(path_str.clone())])
    {
